    ...
    <main class="bg-[#122C4F] text-[#FBF9E4] px-6 py-10 min-h-screen">
        <div class="max-w-3xl mx-auto bg-[#0f203d] rounded-2xl p-6">
            <h1 class="text-2xl font-bold mb-4">
                Edit {{ $scope === 'subs' ? 'Subscription' : 'Product' }} #{{ $product->id }}
            </h1>

            <form wire:navigate method="POST" action="{{ route('admin.products.update', $product) }}" class="space-y-4">
                @csrf @method('PATCH')
                ...
                @if ($scope !== 'subs')
                    <div>
                        ...
                        <select name="category" class="w-full p-2 rounded bg-[#FBF9E4] text-[#122C4F]">
                            ...
                            <option value="women" @selected(old('category', $product->category) === 'women')>women</option>
                            <option value="limited" @selected(old('category', $product->category) === 'limited')>limited
                            </option>
                        </select>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="w-full p-2 rounded bg-[#FBF9E4] text-[#122C4F]">{{ old('description', $product->description) }}</textarea>
